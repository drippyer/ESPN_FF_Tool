...
def getMinMaxScores(league_index):
    ...
    for game in box_scores:
        ...
        teams.append(game.home_team.team_name + " | " + game.away_team.team_name)

    snoozeFest = round(min(scores), 2)
    barnBurner = round(max(scores), 2)
    snoozeTeams = teams[scores.index(min(scores))]
    burnerTeams = teams[scores.index(max(scores))]
    roundUpAwards.add_row(["Barnburner Matchup", burnerTeams, str(barnBurner) + " combined points"])
    roundUpAwards.add_row(["Absolute Snoozefest", snoozeTeams, str(snoozeFest) + " combined points"])
...
def getVictoryMargins(league_index):
    ...
    for game in box_scores:
        ...
        matchups.append(game.home_team.team_name + " | " + game.away_team.team_name)
